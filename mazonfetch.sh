...
_set_info()
{
	...
	inf[1]="${red}  $ql${reset}"
	inf[2]="${green}Distro     ${white}: $f${reset}"
	inf[3]="${blue}OS         ${white}: $f_os${reset}"
	inf[4]="${cyan}Kernel Name${white}: $f_kname${reset}"
	inf[5]="${red}Kernel     ${white}: $f_kv${reset}"
	inf[6]="${purple}Uptime     ${white}: $f_uptime${reset}"
	inf[7]="${violet}Shell      ${white}: $f_shell${reset}"
	inf[8]="${yellow}Resolution ${white}: $f_resolucao${reset}"
 	inf[9]="${white}Desktop    ${white}: $f_desk${reset}"
	inf[10]="${cyan}Fonte      ${white}: $f_font${reset}"
	inf[11]="${green}CPU        ${white}: $f_cpu $f_tcpu${reset}"
	inf[12]="${red}Core(s)    ${white}: $(nproc)${reset}"
	inf[13]="${yellow}GPU        ${white}: $f_gpu${reset}"
	inf[14]="${blue}Mem RAM    ${white}: $(free -h | grep Mem | awk '{print $2 }')${reset}"
	inf[15]="${blue}Mem free   ${white}: $(free -h | grep Mem | awk '{print $4 }')${reset}"
	inf[16]="${purple}Architetura${white}: $f_ach${reset}"
	inf[17]="${white}Browser    ${white}: $f_nave${reset}"
}
